...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        tok_emb = self.transformer.wte(idx)  # token embeddings of shape (T, n_embed)
        x = tok_emb + pos_emb
    ...
    def configure_optimizers(self, weight_decay, learning_rate, device):
        ...
        # i.e. all weight tensors in matmuls + embeddings decay, all bias and layernorms don't
...
# takes tokens, masks, and logits, returns the index of the completion with the lowest loss
...
# pytorch can be serious about it's device v.s. device_type distinction
...
# use for evaluation of generating content
...
if master_process and os.path.exists(log_file):
    os.remove(log_file)

# helper function for HellaSwag
from hellaswag import iterate_examples, render_example

# when to run model evaluation and checkpoint
val_step = 250
...
for step in range(max_steps):
    ...
    if step % val_step == 0 or last_step:
        ...
        if master_process:
            ...
            if (step % checkpoint_step == 0 or last_step) and step > 0:
                ...
                # you might also want to add optimizer.state_dict() and random seed etc.,
                # if you want to resume training more precisely
    ...
    if (step % val_step == 0 or last_step) and step > 0:
        model.eval()
        ...
        while xgen.size(1) < max_length:
            ...
            with torch.no_grad():
                ...
                # topk_probs here becomes (num_return_sequence, 50), topk_indices is (num_return_sequence, 50)
